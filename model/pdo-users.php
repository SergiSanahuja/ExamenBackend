 ...
 */
 ...
 function llistarUsuaris(){
    ...
    $statement = $connexio->prepare('SELECT * FROM users Where NOT Administrador = 1');
...
//Ex13
function resetPassword($hash, $email){

    try{
        $connexio = getConnection();

        $statement = $connexio->prepare('UPDATE users SET password = :md5Hash WHERE email = :email');

        $statement->bindParam('md5Hash', $hash);
